...
public class CacheManagerCheck implements CommandLineRunner {	
	...
	private void getCacheLogAndStatistics()
	{
		...
		cacheNames.stream().forEach(cacheName -> {
			...
			logger.info("The Name of the Cache is  {} ========== ",cacheName);	
			...
			if(nativeCache instanceof org.springframework.cache.ehcache.EhCacheCache){
				org.springframework.cache.ehcache.EhCacheCache scache =  (org.springframework.cache.ehcache.EhCacheCache) cache.getNativeCache();
				logger.debug("The Default Cache is Spring Cache ");
			}
		});
	}
