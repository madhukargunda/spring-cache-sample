...
There are few Cache Libraries alreadt exist , For these Spring provides a adapters

Spring example on In memory cache implementation using spring boot.
This example shows the ConcurrentMap based cache ,ie .Spring Cache - Using SimpleCacheManager


What is the spring default cacheManger if we not provided  as part of classpath :
  ...
  2.Spring uses the SimpleCacheManager as the default Implementation if developer  not provides any provider

     a.SimpleCacheManager which uses the JDK ConcurrentMap ,This is Spring default Generic caching
